...
for (i in 1:length(sub_selec)) {
  ...
  year=format(station[[sub_selec[i]]]$V1,format="%Y")
...
#Uso la funcion join_all la cual une recursivamente data frames en el maximo periodo
#en comun espesificando el tipo de union --> tipe=inner

df2<- join_all(series,by="year",type = "inner")

#Observo que df2, contien NA ,en varias filas por lo tanto no daran un grafico continuo
#Se decide recortar el periodo a uno en donde no contengan datos faltantes .
#utilizo la funcion na.contiguo()el cual encuentra el tramo consecutivo mas largo
#de valores no faltantes.

auxiliar=as.matrix (as.data.frame (lapply (df2, as.numeric)))
miayuda=na.contiguous(auxiliar)

va3=as.numeric(as.character(series[[1]]$year[miayuda[1,1]]))
v4=as.numeric(as.character(series[[1]]$year[miayuda[nrow(miayuda),1]]))

##################################################################
var1=sist_estaciones$rio
nms=midata$Sistema_Hidrico[mas_30[sub_selec]]

for (i in 1:length(series)) {
  series[[i]]$year=as.numeric(as.character(series[[i]]$year))
   v= which(series[[i]]$year==va3)
   l=which(series[[i]]$year==v4)
   misdatos=series[[i]][v:l,]
    
    p<- ggplot(misdatos, aes(x = year ))+
    ...
    ggtitle(paste("Caudales","Sistema Hidrico",nms[i],"/Rio:",var1[i],sep = " "))+ 
    theme(plot.title = element_text(hjust = 0.5))
  
  ggsave(filename=paste0("series.caudales",i,".png"),width = 8,height = 8,dpi = 72)
}
p
